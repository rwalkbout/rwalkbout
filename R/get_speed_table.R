get_speed_table <- function(merge_data){
  gps_speed<-merge_data %>% filter(!str_detect(walk_type.p, "diary")) %>%
    mutate(median_speed.w = round(median_speed.w, 1),
           ...
           speed_flag = ifelse(mean_speed_kmh.p == median_speed.w & is.na(flag_start) & is.na(flag_end), "mean.p = median.walkboutr",""),
           speed_flag = ifelse(mean_speed_kmh.p == mean_speed.w & is.na(flag_start) & is.na(flag_end) & speed_flag!= "mean.p = median.walkboutr", "mean.p = mean.walkboutr", speed_flag),
           speed_flag = ifelse(mean_speed_kmh.p == mean_speed.w & mean_speed_kmh.p == median_speed.w & is.na(flag_start) & is.na(flag_end), "mean.p = median.walkboutr = mean.walkboutr", speed_flag),
